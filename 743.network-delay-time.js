...
var networkDelayTime = function(times, n, k) {
    let arr = new Array(n).fill(Number.MAX_VALUE / 2).map(() => new Array(n).fill(Number.MAX_VALUE / 2))
    //邻接矩阵初始化图
    //因为可能点与点之间无连接，因此初始化为无穷大。此处为避免33行计算溢出，>>2
    for(let i = 0; i < times.length; i++) {
        //根据题目给的信息times，对graph初始化。
        let source = times[i][0] - 1;
        let target = times[i][1] - 1;
        arr[source][target] = times[i][2]
    }
    let visited = new Array(n).fill(false);
    //初始值为false,若访问，则为true。
    let dist = new Array(n).fill(Number.MAX_VALUE / 2)
    //初始化距离表为无穷。
    dist[k - 1] = 0;
    //初始化起始点K-K的距离为0
    for(let j = 0; j < n; j++) {
        ...
        for(let i = 0; i < n; i++) {
            //更新锁死节点与其相邻节点的dist.
            dist[i] = Math.min(dist[i], dist[cur] + arr[cur][i])
        }
    }

    let resule = 0;
    //题目要求，若dist中有未到达的节点，return-1；
    for(let i = 0; i < n; i++) {
        if(dist[i] === Number.MAX_VALUE / 2) {
            return -1
        }
        resule = Math.max(resule, dist[i])
    }
    return resule;
};
